/* ================================================================
 * 德州扑克 AI 训练器 — 游戏引擎（单人模式）
 * 1 真人 vs 8 AI  |  每人 500 初始积分  |  纯单机本地运行
 *
 * 纯函数已提取到 shared/ 目录
 * 浏览器：按依赖顺序加载 <script> 标签
 * Node.js：通过 require 加载所有 shared 模块
 * ================================================================ */

// Node.js: 将 shared 模块导出挂到 globalThis，使裸函数名在模块内可用
// 使用 globalThis 而非 var 声明，避免 hoisting 覆盖浏览器全局变量
...
class PokerGame {
    ...
    startNewHand() {
        ...
        if (this.countActivePlayers() < 2) {
            // 真人被淘汰但还有补码机会 → 不结束
            if (this.eliminatedPlayers.includes(0) && this._rebuysLeft > 0) {
                // 不结束，等待补码
            } else {
    ...
    /** 分析牌面结构（含 GTO 牌面分类） */
    analyzeBoard() {
        return analyzeBoard(this.communityCards);  // → shared/board_analyzer.js
    }

    /** 获取牌面类型对应的策略模板 */
    getBoardStrategy(boardType) {
        return getBoardStrategy(boardType);  // → shared/board_analyzer.js
    }
    ...
    calculateMDF() {
        return calculateMDF(this.players, this.currentBetLevel);  // → shared/board_analyzer.js
    }

    /** 计算下注与底池的比例（用于判断是否超池下注） */
    getBetPotRatio() {
        return getBetPotRatio(this.players, this.currentBetLevel);  // → shared/board_analyzer.js
    }

    /** 计算范围优势：+1=AI有优势，0=中性，-1=对手有优势 */
    getRangeAdvantage(player) {
        return getRangeAdvantage(player.id, this.communityCards, this.preflopRaiserIndex);  // → shared/board_analyzer.js
    }

    /** GTO 下注尺度选择：牌面类型 + 人格 + 行动场景 */
    pickGTOMultiplier(profile, board, situation) {
        return pickGTOMultiplier(profile, board, situation);  // → shared/board_analyzer.js
    }

    /** 评估听牌潜力：花听/两头顺/卡顺，返回 0~0.12 bonus */
    evaluateDrawPotential(player) {
        return evaluateDrawPotential(player.handCards, this.communityCards);  // → shared/board_analyzer.js
    }

    /** 阻断牌评估：量化手牌对对手范围的阻断能力，返回 0-10 分 */
    evaluateBlockers(hand, board) {
        return evaluateBlockers(hand, this.communityCards);  // → shared/board_analyzer.js
    }
    ...
    rebuy() {
        ...
        if (this.phase === 'game_over') {
            ...
            this.message = `💰 补码成功！剩余 ${this._rebuysLeft} 次`;
        }
        this.notifyState();
